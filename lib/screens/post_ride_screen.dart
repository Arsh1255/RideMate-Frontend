...
class _PostRideScreenState extends State<PostRideScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        title: Text("Post a ride", style: TextStyle(color: AppColors.textPrimary, fontWeight: FontWeight.bold, fontSize: 18)),
  ...
  Widget _buildRideNameInput() => Container(padding: const EdgeInsets.symmetric(horizontal: 16), decoration: BoxDecoration(color: AppColors.white, borderRadius: BorderRadius.circular(16), border: Border.all(color: AppColors.border)), child: TextFormField(controller: _nameController, focusNode: _nameFocus, style: const TextStyle(fontSize: 15), decoration: InputDecoration(hintText: "Ride Name (e.g. Office Run)", border: InputBorder.none, hintStyle: TextStyle(color: AppColors.textSecondary))));

  Widget _buildLocationCard() => Container(decoration: BoxDecoration(color: AppColors.white, borderRadius: BorderRadius.circular(16), border: Border.all(color: AppColors.border)), child: Row(children: [const SizedBox(width: 16), _buildRouteGraphic(), Expanded(child: Column(children: [_buildLocField(_pickupController, _pickupFocus, "Pickup location", true), Divider(height: 1, color: AppColors.border), _buildLocField(_dropoffController, _dropoffFocus, "Drop location", false)])), IconButton(onPressed: _swapLocations, icon: const Icon(Icons.swap_vert, color: AppColors.primaryBlue)), const SizedBox(width: 8)]));
  ...
  Widget _buildSectionHeader(String title) => Text(title, style: TextStyle(fontWeight: FontWeight.bold, fontSize: 16, color: AppColors.textPrimary));

  Widget _buildSelectionList() => Column(children: [_selectionRow(Icons.calendar_today_outlined, "Date", DateFormat('EEEE, d MMMM').format(_selectedDateTime), _showDate), Divider(height: 1, color: AppColors.border), _selectionRow(Icons.access_time, "Time", DateFormat('h:mm a').format(_selectedDateTime), _showTime), Divider(height: 1, color: AppColors.border), _selectionRow(Icons.people_outline, "Seats Needed", "$_selectedSeats Seats", _showSeats)]);

  Widget _selectionRow(IconData icon, String label, String val, VoidCallback tap) => ListTile(onTap: tap, contentPadding: EdgeInsets.zero, leading: Icon(icon, color: AppColors.textSecondary, size: 22), title: Text(label, style: TextStyle(color: AppColors.textSecondary, fontSize: 15)), trailing: Row(mainAxisSize: MainAxisSize.min, children: [Text(val, style: const TextStyle(color: AppColors.primaryGreen, fontWeight: FontWeight.w600, fontSize: 15)), Icon(Icons.chevron_right, color: AppColors.textSecondary)]));
                ...
                body: Center(
                  child: Column(
                    ...
                    children: [
                      ...
                      const SizedBox(height: 24),
                      Text("Failed to Create Ride", style: TextStyle(fontSize: 24, fontWeight: FontWeight.bold, color: AppColors.textPrimary)),
                      const SizedBox(height: 8),
                      Padding(
                        padding: const EdgeInsets.symmetric(horizontal: 32),
                        child: Text(error.toString(), textAlign: TextAlign.center, style: TextStyle(fontSize: 14, color: AppColors.textSecondary)),
                      ),
                      const SizedBox(height: 24),
                      ElevatedButton(
